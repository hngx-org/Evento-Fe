...
const Notifications: React.FC<NotificationsProps> = ({ notificationsRef, unreadNotifications, notifications }) => {
  ...
  return (
    <div
      className={`bg-[#fff] font-manropeEL w-fit max-h-[60vh] z-10 border-[#d8d8d8] overflow-y-scroll scrollbar-none rounded-lg shadow-xl ${
        loading ? '' : ''
      }`}
      ref={notificationsRef}
    >
      <div className="pt-2 bg-white-100 px-4 sticky top-0 flex flex-col justify-between">
        ...
        {notifications?.map((notification, index) => (
          <li
            ...
            className={`h-fit py-4 flex px-4 flex-col ${
              notifications.length - 1 === index ? '' : 'border-b border-b-[#DEDEDE]'
          ...
          >
            <div className="flex w-full gap-4 m-auto items-center justify-center align-middle">
              ...
              <div className="flex m-auto items-center gap-2 justify-center w-full flex-col">
                <p className="font-[400] text-[#5B5F5E] text-sm ">{notification.message}</p>
      ...
      {notifications.length === 0 && (
        <div className="text-[#5B5F5E] text-sm font-normal py-4 text-center">No Unread Notifications!!</div>
      )}
    </div>
  );
};
